...
- inspect_network: use no parameter for the 10 most recent XHR/Fetch packets, each with a stable request_id. Use text for a concise plain-text lexical search of already captured response bodies (maximum 512 characters; do not use Lunr syntax or paste the whole task). Search results rank response-body matches before URL/POST-only matches and include up to three matching response fragments. To read one result completely, use its request_id without text; if the result supplies next_cursor, repeat with the same request_id and copy next_cursor unchanged. request_id and text are mutually exclusive, and cursor is valid only with request_id. Complete response bodies are bounded to 25 MiB and may require multiple pages.
