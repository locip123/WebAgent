...
- inspect_network: text searches captured response bodies (plain text, ≤512 characters); request_id reads a result; cursor continues it unchanged. request_id and text are mutually exclusive, and cursor requires request_id. Bodies may be paginated (≤25 MiB).
- find_chart_data_requests: normally omit cursor. Scans the page, saves redacted packets and normalized tables, and returns status, artifact_id, data_dir, summaries, a packet preview, and optional next_cursor. Use cursor only after structured-data failure to inspect more raw fragments; copy next_cursor unchanged.
- call_data_analysis_assistant: analysis_query and data_dir. Copy data_dir exactly from a successful find_chart_data_requests result; do not embed or infer paths in analysis_query. State the complete task-specific calculation in the user's language (dates, filters, entities, metrics, denominator, and output). It reads manifest-listed normalized tables and returns an answer with evidence and provenance.
